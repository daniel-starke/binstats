...
ifeq (,$(DEBUG))
 CPPFLAGS = -I$(INCDIR) -I$(SRCDIR) -isystem $(EXTLIB_SRCDIR)/include
 ifeq (,$(ENABLE_LTO))
  ...
  LDFLAGS = -O2 -s -static -Wl,--allow-multiple-definition -Wl,--gc-sections -L$(LIBDIR) -L$(EXTLIB_SRCDIR)/$(LIBDIRNAME)
 else
  ...
  LDFLAGS = -O2 -s -static -Wl,--allow-multiple-definition -flto -flto-partition=none -fno-devirtualize -ftree-vectorize -fgraphite-identity -fipa-icf -fmerge-all-constants -Wl,--gc-sections -L$(LIBDIR) -L$(EXTLIB_SRCDIR)/$(LIBDIRNAME)
 endif
else
 CPPFLAGS = -I$(INCDIR) -I$(SRCDIR) -isystem $(EXTLIB_SRCDIR)/include
 ...
 LDFLAGS = -static -fno-omit-frame-pointer -Wl,--allow-multiple-definition -L$(LIBDIR) -L$(EXTLIB_SRCDIR)/$(LIBDIRNAME)
endif
ifneq (,$(UNICODE))
 LDFLAGS += -municode
endif
